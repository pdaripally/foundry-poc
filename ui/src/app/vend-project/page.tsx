...
const defaults: FormState = {
  ...
  cost_center: "",
  project_admin_group_oid: "",
  project_user_group_oid: "",
};

export default function VendProjectPage() {
  ...
  const isDevWithRestrictedData =
    form.environment === "dev" && ["confidential", "restricted"].includes(form.data_classification);
  ...
  const canSubmit =
    ...
    form.workload_name &&
    form.cost_center &&
    form.project_admin_group_oid &&
    !isDevWithRestrictedData;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const res = await api.vendProject({
        hub: form.hub as Hub,
    ...
    <div className="max-w-2xl mx-auto space-y-5">
      <div>
        <h2 className="page-heading flex items-center gap-2">
          <FolderPlus size={22} className="text-emerald-600" /> Vend Foundry Project
        </h2>
        <p className="page-subheading">
          Runs OPA policy checks then deploys a new Foundry project with RBAC isolation.
          Triggers <code className="rounded bg-kpmg-gray px-1 py-0.5 text-xs font-mono">vend-foundry-project.yml</code>.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-2 mt-1">
                {(["amr", "emea", "apac"] as Hub[]).map((h) => (
                  <button
                    ...
                    type="button"
                    onClick={() => set("hub", h)}
                    className={`select-card py-3 font-bold text-sm ${
                      form.hub === h ? "select-card-active text-kpmg-blue" : "select-card-inactive text-kpmg-black"
                    }`}
                  >
                    {h.toUpperCase()}
                  </button>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3 mt-1">
                {[
                  { value: "mfs" as Tier, label: "MFS Shared", desc: "Member Firm Shared — multi-tenant member firm workloads" },
                  { value: "tax" as Tier, label: "GF Tax", desc: "Global Function Tax — tax domain AI workloads" },
                ].map((t) => (
                  <button
                    key={t.value}
                    type="button"
                    ...
                    className={`select-card text-left ${
                      form.subscription_tier === t.value ? "select-card-active" : "select-card-inactive"
                    }`}
                  >
                    <p className={`text-sm font-bold ${form.subscription_tier === t.value ? "text-kpmg-blue" : "text-kpmg-black"}`}>
                      {t.label}
                    </p>
                    <p className="text-xs text-kpmg-gray-dark mt-0.5 leading-tight">{t.desc}</p>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-2 mt-1">
                {(["dev", "staging", "prod"] as Env[]).map((env) => (
                  <button
                    key={env}
                    type="button"
            ...
            <div>
              ...
              <div className="grid grid-cols-4 gap-2 mt-1">
                {(Object.keys(dataClassConfig) as DataClass[]).map((dc) => (
                  <button
                    key={dc}
                    type="button"
                    onClick={() => set("data_classification", dc)}
              ...
              {isDevWithRestrictedData && (
                <p className="mt-1 text-xs font-semibold text-red-600">
                  ✕ {form.data_classification} data is not allowed in a dev environment (OPA policy).
                </p>
              )}
            </div>
            ...
            {projectNamePreview && (
              <div className="rounded-lg bg-kpmg-gray border border-kpmg-gray-mid px-3 py-2">
                <p className="text-[10px] font-bold uppercase tracking-widest text-kpmg-gray-dark mb-0.5">
        ...
        {result && (
          <Alert
            variant="success"
            title="Project vending triggered"
        ...
        <div className="flex justify-end">
          <button type="submit" className="btn-primary" disabled={loading || !canSubmit}>
            ...
            {loading ? "Triggering…" : "Vend Project"}
          </button>
        </div>
      </form>
    </div>
